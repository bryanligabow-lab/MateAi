...
export default async function ServicePage({params,searchParams}:{params:Promise<{slug:string}>;searchParams:Promise<{lang?:string}>}){const slug=(await params).slug;const language:Language=(await searchParams).lang==="en"?"en":"es";if(slug==="social-media")redirect(`/services/web-chatbots?lang=${language}`);const service=getService(slug);if(!service||service.slug==="web-chatbots")notFound();const content=service[language];const t=shared[language];return <><section className="service-hero"><div className="service-hero-orb"/><div><Link className="service-back" href="/">← {t.back}</Link><span className="section-kicker">{t.kicker} · {service.icon}</span><h1>{content.name}</h1><p>{content.description}</p><Link className="button button-primary" href={`/book-demo?lang=${language}`}>{t.cta} <span>↗</span></Link></div></section><section className="section service-detail"><span className="section-kicker">{t.discover}</span><h2>{t.headline}</h2><p className="section-intro">{t.body}</p><div className="service-detail-grid">{t.items.map((item,index)=><div key={item}><b>0{index+1}</b><h3>{item}</h3></div>)}</div></section></>}
