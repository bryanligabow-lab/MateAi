...
export function ServiceGateway(){const [phase,setPhase]=useState<Phase>("preloader");const [word,setWord]=useState(0);const [language,setLanguage]=useState<Language>("en");const t=copy[language];
 useEffect(()=>{history.scrollRestoration="manual";window.scrollTo(0,0);setWord(0);setPhase("preloader");document.body.style.overflow="hidden";const interval=window.setInterval(()=>setWord(value=>(value+1)%welcomes.length),430);const timer=window.setTimeout(()=>setPhase("language"),2800);return()=>{window.clearInterval(interval);window.clearTimeout(timer);document.body.style.overflow=""}},[]);
 const chooseLanguage=(value:Language)=>{setLanguage(value);setPhase("services");window.dispatchEvent(new Event("mateai:language-selected"));window.scrollTo(0,0)};
 ...
 if(phase==="language")return <section className="choice-screen"><div className="choice-glow"/><div className="choice-shell"><span className="choice-brand">MATEAI</span><span className="section-kicker">Welcome · Bienvenidos</span><h1>Choose your language.<br/><em>Elige tu idioma.</em></h1><div className="choice-actions"><button onClick={()=>chooseLanguage("en")}><small>01</small><strong>English</strong><span>Continue ↗</span></button><button onClick={()=>chooseLanguage("es")}><small>02</small><strong>Español</strong><span>Continuar ↗</span></button></div></div></section>;
